...
SELECT vendor_id,booth_number, count(vendor_id) as count_of_vendor_booth_assignments
...
SELECT c.customer_last_name,c.customer_first_name,SUM(quantity*cost_to_customer_per_qty) as total_amount_spent
FROM customer c
...
SELECT customer_id, strftime('%m',market_date) as month, strftime('%Y',market_date) as year,
SUM(quantity*cost_to_customer_per_qty) as money_each_customer_spent
FROM customer_purchases
